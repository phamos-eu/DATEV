...
class TestDatevSalesInvoiceGrouping(TestCase):
	...
	def test_groups_sales_invoice_rows_only_when_account_and_tax_match(self):
		...
		with (
			...
			patch(
				"datev.datev.report.datev.datev.frappe.db.get_value",
				side_effect=["10001"],
			),
		):
	...
	def test_groups_receive_payment_entry_rows_with_blank_bu_schluessel(self):
		...
		with (
			...
			patch(
				"datev.datev.report.datev.datev.get_party_account_number",
				return_value="10001",
			),
			patch(
				"datev.datev.report.datev.datev.get_payment_entry_account_number",
				return_value="1200",
			),
		):
